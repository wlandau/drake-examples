...
See the [corresponding chapter of the user manual](https://books.ropensci.org/drake/packages.html) for the details. If you wish to tinker with this example yourself, you can generate the [underlying code files](https://github.com/ropensci/drake/tree/master/inst/examples/packages) with `drake_example("packages")` and then step through `run.R`. This `report.Rmd` file does not stand on its own. It is meant to be called through `run.R`. In fact, the `run.R` file has instructions (comments) to walk you through the project.
