...
If you wish to tinker with the example yourself, you can generate the [underlying code files](https://github.com/ropensci/drake/tree/main/inst/examples/gsp) with `drake_example("gsp")` and then step through `run.R`. This `report.Rmd` file does not stand on its own. It is meant to be called through `run.R`. In fact, `run.R` has instructions (comments) to walk you through the project.
